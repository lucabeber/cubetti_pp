...
%     figure;
    ...
    for i=1:20
        ...
        force = -(force_zt - mean(force_zt(1:min_elements)));
        warning('off', 'all');
        error = fminsearch(@(x)res_minimization(x,force,data,position_realt,max_pen,min_elements,n),error);
        warning('on', 'all');
        k = 1;
        flag = true;
        while(flag)
...
%         subplot(4,5,i);
%         plot(force(start:finish-1))%  + ls_coeff(2)*X(:,2))
%         hold on 
%         plot(ls_coeff(1)*X(:,1))
%         hold off 
%         legend(["est","ft"])
